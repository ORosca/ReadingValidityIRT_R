---
title: "Missing Data Analysis for Theta-Scores Obtained via 124 Items of IES DAACS Reading Assessment, May 2022 - May 2023, umgc1-and-ua2 Combined Sample, AnSamp2 (n = 1563) and Would-be-sample 2022 (n = 2620)"
author: "Oxana Rosca"
date: "`r Sys.Date()`"
output:
  word_document:
...
The purpose of this document is to analyze the missing data in the Analytic Sample 2 for the DAACS Reading Assessment in 2022. The Analytic Sample 2 (AnSamp2) consists of the first-attempt scores from all non-speedy treatment respondents who took the Reading Assessment in 2022, during their first semester, and had data on age. The Analytic Sample 2 is a subset of the Would-be Analytic Sample 2 (sampW22), which includes all newly enrolled non-speedy, "treatment-group" undergraduate students who took the DAACS Reading Assessment in 2022. The missing data analysis will focus on the missingness in the "Age" variable and its impact on the Theta-scores in the combined sample and the subsamples of UMGC1 and UA2 respondents.
...
```{r}
#load("~/Dropbox (Hunter College)/DAACS-Validity/Analyses/dataPrep/Reading_dataClean-umgc1ua2_5.RData")
#load("D:/OneDrive - University at Albany - SUNY/My DAACS/read/Reading_dataClean-umgc1ua2_5.RData")
load("C:/Users/orosc/OneDrive - University at Albany - SUNY/My DAACS/read/read_dataClean-UMGC1UA2_5.RData")
```
...
### Removed 11 "control" students (contamination)
...
2620 not-control-group students
```{r}
...
```{r}
colSums(is.na(read.items_AnSamp2[ , c("Theta_read124", "Age_d24", "Age", "college", "gender", "Military", "Pell", "transfer")]))
```

## Theta-scores Distribution in three samples; Tendency Measures
...
# Generate the summary table
  readThetas_allSamps_summary <- 
  dfSummary(readBifactor2PL_umgc1ua2_all_Thetas[ , c(2:4,6:8,10:12)], 
            varnumbers = F, round.digits = 2)
  
# To display the summary in the Viewer
#view(readThetas173_summary)
  
# To render the summary in the Markdown pane
  print(readThetas_allSamps_summary, method = "render")
  
# To save as an HTML file:
stview(readThetas_allSamps_summary, file = "readThetas_allSamps_summary.html")
```    

#### No correlation with Demographic Variables: Pmax = 0.1, Correlation plots, AnSamp2
Thetas do not correlate with any of 6 demographic variables
```{r}
# library(png)
# library(psych)
# Generate the pairs plot with larger font sizes and no title
readTheta124demogr_AnSamp2_pairsPlot <- function() {
  pairs.panels(
    read.items_AnSamp2[ , c("Theta_read124", "Age_d24", "Age", 
...
png("readTheta124demogr_AnSamp2_pairsPlot.png", width = 800, height = 600)
readTheta124demogr_AnSamp2_pairsPlot()
dev.off()
...
scatterplot_ageTheta124_AnSamp2 <- ggplot(data = read.items_AnSamp2, aes(x = Age, y = Theta_read124)) +
  geom_point(na.rm = TRUE) +
  ggtitle("Theta-scores and Age")

scatterplot_collegeTheta124_AnSamp2 <- ggplot(data = read.items_AnSamp2, aes(x = Age, y = Theta_read124)) +
  geom_point() +
  facet_wrap(~college, ncol = 2) +
  theme(legend.position = "bottom") +
  ggtitle("Theta-scores and College")

scatterplot_genderTheta124_AnSamp2 <- ggplot(data = read.items_AnSamp2, aes(x = Age, y = Theta_read124)) +
  geom_point() +
  facet_wrap(~gender, ncol = 2) +
  theme(legend.position = "bottom") +
  ggtitle("Theta-scores and Gender")

scatterplot_militaryTheta124_AnSamp2 <- ggplot(data = read.items_AnSamp2, aes(x = Age, y = Theta_read124)) +
  geom_point() +
  facet_wrap(~Military, ncol = 2) +
  theme(legend.position = "bottom") +
  ggtitle("Theta-scores and Military Status")

scatterplot_pellTheta124_AnSamp2 <- ggplot(data = read.items_AnSamp2, aes(x = Age, y = Theta_read124)) +
  geom_point() +
  facet_wrap(~Pell, ncol = 2) +
  theme(legend.position = "bottom") +
  ggtitle("Theta-scores and SES (Pell)")

scatterplot_transferTheta124_AnSamp2 <- ggplot(data = read.items_AnSamp2, aes(x = Age, y = Theta_read124)) +
  geom_point() +
  facet_wrap(~transfer_rec, ncol = 2) +
  theme(legend.position = "bottom") +
  ggtitle("Theta-scores and Transfer Status")

scatterplot_ethnicityTheta124_AnSamp2 <- ggplot(data = read.items_AnSamp2, aes(x = Age, y = Theta_read124)) +
  geom_point() +
...
age_grob_tmp <- ggplotGrob(scatterplot_ageTheta124_AnSamp2)
college_grob_tmp <- ggplotGrob(scatterplot_collegeTheta124_AnSamp2)
gender_grob_tmp <- ggplotGrob(scatterplot_genderTheta124_AnSamp2)
military_grob_tmp <- ggplotGrob(scatterplot_militaryTheta124_AnSamp2)
pell_grob_tmp <- ggplotGrob(scatterplot_pellTheta124_AnSamp2)
transfer_grob_tmp <- ggplotGrob(scatterplot_transferTheta124_AnSamp2)
ethnicity_grob_tmp <- ggplotGrob(scatterplot_ethnicityTheta124_AnSamp2)

# Include the pairs plot as an image grob
pairs_image_grob_tmp <- rasterGrob(png::readPNG("readTheta124demogr_AnSamp2_pairsPlot.png"), interpolate = TRUE)

# Dynamically generate the plot title with sample size
plot_title_tmp <- 
  paste0("Relationship between read Theta Scores and Demographic Variables, 
         UMGC1 and UA2, n = ", nrow(read.items_AnSamp2))
...
pdf("readUMGC1UA2_Correlations_Theta_Scores with Demographics.pdf", width = 16, height = 8) # Landscape format
grid.arrange(
  main_title_tmp, 
...
```{r}
scatterplot_ageTheta124_AnSamp2 
scatterplot_collegeTheta124_AnSamp2 
scatterplot_genderTheta124_AnSamp2 
scatterplot_militaryTheta124_AnSamp2 
scatterplot_pellTheta124_AnSamp2 
scatterplot_transferTheta124_AnSamp2 
scatterplot_ethnicityTheta124_AnSamp2
```
...
var_selection_tmp <- names(read.items_sampW22)[ c(2:5, 9, 10, 12, 14:16, 19, 20, 22:205)]
...
variablesSummary(read.items_sampW22, "readItems_sampW22_SummaryTable")
```
...
In both groups the Theta_read124 are not normally distributed. 
```{r}
# Shapiro-Wilk normality test for each group
shapiro.test(read.items_sampW22$Theta_read124
             [read.items_sampW22$Age_missing=="Missing"])
shapiro.test(read.items_sampW22$Theta_read124
             [read.items_sampW22$Age_missing=="Not Missing"])
```

#### Wilcoxon rank sum test: W = 781620, p = 05804; 
...
The p-value is greater than 0.05, hence, we can't reject the null hypothesis, suggesting a no impact of missing values in Age on the target variable of Theta-scores.
Welch Two Sample t-test: p-value = 0.038
mean in group Missing               mean in group Not Missing 
      -0.029                       0.032
```{r}
wilcox_test_result <- 
  wilcox.test(Theta_read124 ~ Age_missing,read.items_sampW22)
print(wilcox_test_result)
# t_test_result <- 
t.test(Theta_read124 ~ Age_missing, read.items_sampW22)
# print(t_test_result)
```

#### Effect Size for Median Differences is "Negligible":  Cliff's Delta = -0.04, 95%CI[-0.09, 0.002]
...
#  Cliff's Delt effect size
cliff_delta_result <- cliff.delta(
  Theta_read124 ~ Age_missing, 
...
### umgc1: MCAR
The missingness in Age does not seem to have an impact on Theta_read124 for the umgc1 group, according to the Wilcoxon rank-sum test. 
...
#### Wilcoxon rank sum test: 453004, p-value = 0.69 
...
Welch Two Sample t-test: p-value = 0.73
...
### ua2: MCAR
The missingness in Age does not seem to have an impact on Theta_read124 for the ua2 group, according to the Wilcoxon rank-sum test.

#### Shapiro-Wilk normality test for "missing" group: p-value = 0.028; for  "non-missing" group: p-value < 0.001
In both groups, the Theta_read124 are not normally distributed. 
Shapiro-Wilk normality test: in both groups the Theta_read124 are not normally distributed. 
```{r}
# Shapiro-Wilk normality test for each group
shapiro.test(read.items_sampW22$Theta_read124
             [ c(read.items_sampW22$college==
...
#### Wilcoxon rank sum test: 11830, p-value = 0.55 
Because the data is not normally distributed, we use a non-parametric Wilcoxon rank sum test with continuity correction; we can't reject the null hypothesis, suggesting a no impact of missing values in Age on the target variable of Theta-scores.
...
explanatory_num <- c("Age", "credits_attempted_f22", "credits_earned_f22", 
            "credits_transferred", "gpa_term_f22", "Theta_read124")
```
...
Analytic Sample 2 (read.items_AnSamp2) is a subset of sampW22: the first-attempt scores from all non-speedy treatment respondents who took Reading Assessment in 2022, during their first semester, AND have data on age variable.
Similarly to sampW22 and AnSamp22D, all discrepancies between the two versions of AnSamp2 were due to the new five columns ("itemsTaken", "readCompletionYear", "Theta_read124", "Theta_read180" , and "Age_missing")
```{r}

read.items_AnSamp2_newer_tmp<-
  read.items_samp22D[complete.cases(read.items_samp22D[,c("Age_d24","ethnicity","gender", "Military", "Pell", "transfer")]),]
...
# Close-to-normal Distribution of Theta Scores using 124 Items
The histograms of Theta124-scores for all four samples show a negatively skewed distribution. The density plot further illustrates almost fully overlapping distribution of Theta-scores across the three samples.
...
# Create histograms
ThetaR124_hist_AnSamp1 <- create_histogram(
...
ThetaR124_hist_sampW22 <- create_histogram(
  data = read.items_sampW22,
  variable = "Theta_read124",
  mean_value = mean(read.items_sampW22$Theta_read124),
  sd_value = sd(read.items_sampW22$Theta_read124),
  title_prefix = "sampW22: Theta-scores"
)

ThetaR124_hist_samp22D <- create_histogram(
  data = read.items_samp22D,
  variable = "Theta_read124",
  mean_value = mean(read.items_samp22D$Theta_read124),
  sd_value = sd(read.items_samp22D$Theta_read124),
  title_prefix = "samp22D: Theta-scores"
)

ThetaR124_hist_AnSamp2 <- create_histogram(
...
ThetaR124_hist_AnSamp1 
ThetaR124_hist_sampW22 
ThetaR124_hist_samp22D
ThetaR124_hist_AnSamp2
```
...
ThetaR124_densityPlot <- ggplot(combined_data_tmp, aes(x = Theta_read124, color = Sample, fill = Sample)) +
  geom_density(alpha = 0.4) +
  labs(
    title = "Density Plot: Theta124-scores",
    x = "Theta-scores",
    y = "Density"
  ) +
  theme_minimal() +
...
pdf("Read Theta_Scores124_Histograms_Density.pdf", width = 11, height = 8.5)
grid.arrange(
  ThetaR124_densityPlot,
  ThetaR124_hist_AnSamp1,
  ThetaR124_hist_sampW22,
  ThetaR124_hist_AnSamp2,
   nrow = 2
...
pdf("Read Theta_Scores124_Histograms.pdf", width = 11, height = 8.5)
grid.arrange(
  ThetaR124_hist_AnSamp1,
  ThetaR124_hist_sampW22,
  ThetaR124_hist_samp22D,
...
```{r}
sort(read.items_AnSamp1$Theta_read124, decreasing = TRUE)[1:10]
sort(read.items_AnSamp2$Theta_read124, decreasing = TRUE)[1:10]
sort(read.items_sampW22$Theta_read124, decreasing = TRUE)[1:10]
```

# Save all data in a single file 
```{r}
#save.image("D:/Dropbox/DAACS-Validity/Analyses/read/Reading_dataClean-umgc1ua2_6.RData")
save.image("C:/Users/orosc/OneDrive - University at Albany - SUNY/My DAACS/read/read_dataClean-umgc1ua2_6.RData")
```